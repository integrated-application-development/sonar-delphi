...
public class BlockDataFlowVisitor {
  ...
  private void registerUsage(DelphiNode declaration) {
    if (!(declaration instanceof LiveVariable)) {
      return;
    }
  ...
  private void registerAssignment(DelphiNode reference) {
    if (!(reference instanceof LiveVariable)) {
      return;
    }
  ...
  public BlockDataFlowVisitor setOnAssign(Consumer<LiveVariable> onAssign) {
   ...
   */
  public BlockDataFlowVisitor setOnReference(Consumer<LiveVariable> onReference) {
  ...
  private void handleAnonymousRoutine(AnonymousMethodNode reference) {
    if (!(reference instanceof AnonymousMethodNodeImpl)) {
      return;
    }
    ...
    Set<LiveVariable> referencedVariables =
  ...
  private void handleRaiseStatement(RaiseStatementNode raiseStatement) {
    if (raiseStatement.getRaiseExpression() != null) {
      return;
    }

    DelphiNode current = raiseStatement.getParent();
    while (current != null) {
      if (current instanceof TryStatementNode) {
        return;
      }
  ...
  private static boolean hasVariableReference(LiveVariable variable) {
    if (variable instanceof NameDeclarationNode) {
      return true;
    }
