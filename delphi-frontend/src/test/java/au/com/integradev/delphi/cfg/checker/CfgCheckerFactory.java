...
public class CfgCheckerFactory {
  ...
  public static class CfgChecker {
    ...
    public void check(final ControlFlowGraphImpl cfg) {
      ...
      final List<Block> blocks = new ArrayList<>(cfg.getBlocks());
      final Block exitBlock = blocks.remove(blocks.size() - 1);
      for (final Block block : blocks) {
        checkerIterator.next().check(block);
        checkLinkedBlocks(block.getId(), "Successor", cfg.getBlocks(), block.getSuccessorBlocks());
        //        checkLinkedBlocks(block.getId(), "Predecessors", cfg.getBlocks(),
        // block.getPredecessors());
      }
      assertThat(exitBlock.getElements()).isEmpty();
      assertThat(exitBlock.getSuccessorBlocks()).isEmpty();
  ...
  //
  //    BlockChecker(
  //        final Class<? extends DelphiNode> kind, Predicate<DelphiNode> predicate, final int...
  // ids) {
  //      successors(ids);
