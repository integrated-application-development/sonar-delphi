...
public class DelphiASTAnalyzerTest {
  ...
  public void testFunctions() {
    String[] names = { "tdemo.bshowtrackerclick", "tdemo.getfunction", "tmyclass.myprocedure", "tmyclass.setsomething",
    ...
    for (FunctionInterface func : functions) {
      ...
      assertEquals(complexities[index], func.getComplexity()); // checking complexities
      assertEquals(statements[index], func.getStatements().size()); // checking statements
      assertEquals("Called functions at " + names[index], calledFunc[index], func.getCalledFunctions().length); // chcecking called
                                                                                                                // functions
      assertEquals(names[index], global[index], func.isGlobal()); // checking if function is global
      assertEquals(names[index], numArgs[index], func.getArguments().length); // number of arguments
    ...
    assertEquals("DECLARATIONS COUNT", 4, declarations); // how many declarations
  ...
  public void testClasses() {
    ...
    for (ClassInterface cl : classes) {
      ...
      for (ClassFieldInterface field : cl.getFields()) { // checking class fields
