...
public class DelphiPmdSensorTest {
  ...
  public void analyseTest() {
    ...
    org.sonar.api.batch.sensor.issue.Issue[] issues = sensorContext.allIssues()
        .toArray(new org.sonar.api.batch.sensor.issue.Issue[0]);
    ...
    for (int i = 0; i < issues.length; ++i) {
      ...
      System.out.println(
          issue.ruleKey().rule() + ":" + issue.primaryLocation().textRange().start().line());

      assertThat(ruleData[i].toString(), issue.ruleKey().rule(), is(ruleData[i].getName()));
      assertThat(ruleData[i].toString(), issue.primaryLocation().textRange().start().line(),
          is(ruleData[i].getLine()));
    }
  }
}
