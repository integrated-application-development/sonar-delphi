...
public class TooManySubProceduresRule extends DelphiRule {
    ...
    /**
     * This rule adds violations when there are too many defined sub procedures in a defined implementation block, as
     * defined by a user defined LIMIT value.
     *
     * Subprocedures can be nested, so this rule attempts to account for that by searching at multiple depths in the
     * tree.
    ...
    public void visit(DelphiPMDNode node, RuleContext ctx) {
        ...
        if (node.getType() == DelphiLexer.IMPLEMENTATION) {
            ...
            if (children != null) { // Some implementation nodes may not have any code yet, hence no children
                for (int i = 0; i < children.size(); i++) {

                    if ((children.get(i).toString().equals("procedure")) || (children.get(i).toString().equals("function"))) {
                        subProcedureDepth += 1;
