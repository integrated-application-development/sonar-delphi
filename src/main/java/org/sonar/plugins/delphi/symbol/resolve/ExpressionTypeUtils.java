...
public final class ExpressionTypeUtils {
  private ExpressionTypeUtils() {
    // utility class
  }

  public static Type resolve(BinaryExpressionNode expression) {
  ...
  public static Type resolve(UnaryExpressionNode expression) {
    ExpressionNode operand = expression.getOperand();
    UnaryOperator operator = expression.getOperator();
    if (operator == UnaryOperator.ADDRESS) {
      return untypedPointer();
    } else {
      return resolveOperatorType(List.of(operand), operator);
    }
  }

  public static Type resolve(PrimaryExpressionNode expression) {
  ...
  private static Type resolveOperatorType(List<ExpressionNode> argumentNodes, Operator operator) {
    ...
    for (InvocationArgument argument : arguments) {
      ...
      collectOperatorMethods(argument.getType(), operator).forEach(resolver::addCandidate);
    }

    operator.getIntrinsicOperatorSignatures().stream()
  ...
  private static Set<InvocationCandidate> collectOperatorMethods(Type type, Operator operator) {
    Set<Invocable> invocables = Collections.emptySet();
    type = TypeUtils.findBaseType(type);

    if (type.isStruct()) {
      StructType structType = ((StructType) type);
      invocables =
          structType.typeScope().getMethodDeclarations().stream()
      ...
      if (pointer.allowsPointerMath()) {
        if (operator == BinaryOperator.ADD) {
          invocables = BinaryOperator.createPointerMathAddSignaturesFor(pointer);
        } else if (operator == BinaryOperator.SUBTRACT) {
          invocables = BinaryOperator.createPointerMathSubtractSignaturesFor(pointer);
        }
      }
    } else if (type.isVariant()) {
      if (operator instanceof BinaryOperator) {
        invocables = BinaryOperator.getVariantSignatures((BinaryOperator) operator);
      } else if (operator instanceof UnaryOperator) {
        invocables = UnaryOperator.getVariantSignatures((UnaryOperator) operator);
      }
    }

    return invocables.stream()
        .map(InvocationCandidate::new)
        .collect(Collectors.toUnmodifiableSet());
  }
  ...
  private static Type handleTyped(Typed typed) {
  ...
  private static Type handleNameReference(NameReferenceNode reference) {
    Type type = null;
