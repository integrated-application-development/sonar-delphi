...
/**
 * Creates concrete compiler directives based on a given string. Example: {$include unit.pas} should create a IncludeDirective instance
 * 
 */
public class CompilerDirectiveFactory {
   ...
   * 
   * @param data
   *          String including some compiler directive
   * @param startPosition
   *          Start position to look for a directive
   * @param endPosition
   *          End position of a directive
   * @return concrete compiler directive class
   * @throws CompilerDirectiveFactorySyntaxException
   *           when no compiler directive could be created
  ...
  public CompilerDirective create(String data, int startPosition, int endPosition)
    ...
    switch (type) {
      ...
      default:
        throw new CompilerDirectiveFactoryUnsupportedDirectiveException("Not implemented directive name: " + directiveName);
    }
  }
  ...
  private int getDirectiveLastChar(String data, int startPosition) throws CompilerDirectiveFactorySyntaxException {
    ...
    if (pos == -1) {
      throw new CompilerDirectiveFactorySyntaxException("No closing bracket for compiler directive from: " + startPosition + " in: " + data);
    }
    return pos;
  }
  ...
  private String getName(String data, int startPos) throws CompilerDirectiveFactorySyntaxException {
    if (startPos > -1) {
      ...
      if (itemPos < endPos && itemPos > -1) { // we have an item in directive
