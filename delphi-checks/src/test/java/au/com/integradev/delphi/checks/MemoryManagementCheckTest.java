...
class MemoryManagementCheckTest extends CheckTest {
  @BeforeEach
  void setup() {
    var rule = getRule(MemoryManagementCheck.class);

    var memoryFunctions = rule.getProperty(MemoryManagementCheck.MEMORY_FUNCTIONS.name());
    Objects.requireNonNull(memoryFunctions).setValue("Test.TMemory.Manage<T>");

    var whitelist = rule.getProperty(MemoryManagementCheck.WHITELISTED_NAMES.name());
