...
public class RedundantJumpCheck extends DelphiCheck {
  ...
  public DelphiCheckContext visit(RoutineImplementationNode routine, DelphiCheckContext context) {
    ControlFlowGraphImpl cfg = ((RoutineImplementationNodeImpl) routine).cfg();
    if (cfg != null) {
      cfg.blocks().forEach(block -> checkBlock(block, context));
    }
  ...
  public DelphiCheckContext visit(AnonymousMethodNode routine, DelphiCheckContext context) {
    ...
    if (compoundStatementNode != null) {
      ControlFlowGraphImpl cfg = new ControlFlowGraphImpl(compoundStatementNode.getStatements());
      cfg.blocks().forEach(block -> checkBlock(block, context));
    }
  ...
  private void checkBlock(Block block, DelphiCheckContext context) {
    ...
    if (successorWithoutJump != null
        && isContinueOrExit(terminator)
        && !isExitWithExpression(terminator)) {
  ...
  private static Block getFinallyBlock(Block block) {
    return block.successors().stream().filter(Block::isFinallyBlock).findFirst().orElse(null);
  }

  private static boolean isInViolatingTryFinally(Block finallyBlock) {
    while (finallyBlock.successors().size() == 1) {
      Block finallySuccessor = finallyBlock.successors().iterator().next();
      if (!finallySuccessor.isFinallyBlock()) {
        break;
      }
      finallyBlock = finallySuccessor;
    }
    return finallyBlock.successors().size() == 1
        && finallyBlock.successors().iterator().next().successors().isEmpty();
  }
