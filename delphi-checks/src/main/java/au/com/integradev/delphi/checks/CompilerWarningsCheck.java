...
@DeprecatedRuleKey(ruleKey = "CompilerWarningsRule", repositoryKey = "delph")
@Rule(key = "CompilerWarnings")
public class CompilerWarningsCheck extends DelphiCheck {
  private static final String MESSAGE = "Suppressing warnings is not allowed.";
  ...
  private static boolean isViolation(CompilerDirective directive) {
    if (directive instanceof SwitchDirective) {
      ...
      return switchDirective.kind() == SwitchKind.WARNINGS && !switchDirective.isActive();
    }

    if (directive instanceof WarnDirective) {
